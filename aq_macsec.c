...
static int aq_apply_txsc_cfg(struct aq_nic_s *nic, const int txsc_idx)
{
	struct aq_macsec_txsc *aq_txsc = &nic->macsec_cfg->aq_txsc[txsc_idx];
	const struct macsec_secy *secy = aq_txsc->sw_secy;
	struct macsec_tx_sa *tx_sa;
...
static int aq_apply_rxsc_cfg(struct aq_nic_s *nic, const int rxsc_idx)
{
			     ...
			     const struct macsec_secy *secy)
{
	...
	txsc_idx = aq_get_txsc_idx_from_secy(nic->macsec_cfg, secy);
	if (txsc_idx >= 0)
		aq_apply_txsc_cfg(nic, txsc_idx);
	...
	for (rx_sc = rcu_dereference_bh(secy->rx_sc); rx_sc && rx_sc->active;
		...
		ret = aq_apply_rxsc_cfg(nic, rxsc_idx);
		if (ret)
			return ret;
	}
